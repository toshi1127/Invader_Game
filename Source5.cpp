...
public:
	...
	void move(double *x, double *z, double *X, double *XX, double *XXX, double *XXXX, double *XXXXX){
		if (counter % 1000 == 0){//まずはじめにどっちに移動するかで場合分けする。
			if (cnt % 2 != 0){
				if (*X >= 3.9){//一番左が満たしたら
					cnt++;
					*XXXXX = 0.3;
...
void display(){
	...
	if (wall2->exist) wall2->draw();



	double d = 0.6;

	Color* C2 = new Color(1, 0, 0.5);//ライフポイント関係byづかさん
